	...
	def jump(self, state: State, transient: Transient, transition: int, branch: int, assignment_index: int, target_state: State, target_transient: Transient) -> None:
		if assignment_index == 0:
			...
			if location == 0:
				...
				if transition == 0:
					if branch == 0:
						target_state.count = min((state.count + 1), 1000000)
...
class Network(object):
	...
	def __init__(self):
		...
		self.properties = [Property("DidWeCoC", PropertyExpression("exists", [PropertyExpression("eventually", [PropertyExpression("ap", [0])])]))]
