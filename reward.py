...
# States
class State(object):
	__slots__ = ("success", "count", "Main_location")
	...
	def __eq__(self, other):
		return isinstance(other, self.__class__) and self.success == other.success and self.count == other.count and self.Main_location == other.Main_location
...
class MainAutomaton(object):
	...
	def __init__(self, network: Network):
		...
		self.transition_counts = [2, 1, 1, 1, 0, 1]
		self.transition_labels = [[1, 2], [1], [0], [3], [], [0]]
	...
	def jump(self, state: State, transient: Transient, transition: int, branch: int, assignment_index: int, target_state: State, target_transient: Transient) -> None:
		if assignment_index == 0:
			...
			if location == 0:
				...
				if transition == 0:
					if branch == 0:
						target_state.count = min((state.count + 1), 7)
...
class Transition(object):
	...
	def __init__(self, label = 0, transitions = [-1]):
		self.label = label
...
class Network(object):
	__slots__ = ("network", "transition_labels", "sync_vectors", "properties", "aut_Main")
	...
	def __init__(self):
		...
		self.aut_Main = MainAutomaton(self)
	
	def get_initial_state(self) -> State:
		state = State()
		state.success = False
		state.count = 0
		self.aut_Main.set_initial_values(state)
		return state
	
	def _get_initial_transient(self) -> Transient:
		transient = Transient()
		transient.cost = 0
		self.aut_Main.set_initial_transient_values(transient)
		return transient
	...
	def _get_transient_value(self, state: State, transient_variable: str):
		result = self.aut_Main.get_transient_value(state, transient_variable)
		if result is not None:
			return result
		if transient_variable == "cost":
			return 0
		return None
	...
	def get_transitions(self, state: State) -> List[Transition]:
		...
		transitions = []
		trans_Main = [[], [], [], []]
		transition_count = self.aut_Main.get_transition_count(state)
		for i in range(transition_count):
			if self.aut_Main.get_guard_value(state, i):
				trans_Main[self.aut_Main.get_transition_label(state, i)].append(i)
		...
		for i in range(len(transitions)):
			transitions[i] = Transition(transitions[i][1], transitions[i])
	...
	def get_branches(self, state: State, transition: Transition) -> List[Branch]:
		...
		if transition.transitions[0] != -1:
			existing = len(combs)
			branch_count = self.aut_Main.get_branch_count(state, transition.transitions[0])
			for i in range(1, branch_count):
				probability = self.aut_Main.get_probability_value(state, transition.transitions[0], i)
				for j in range(existing):
					combs.append(combs[j][:])
					combs[-1][0] = i
					probs.append(probs[j] * probability)
			probability = self.aut_Main.get_probability_value(state, transition.transitions[0], 0)
			for i in range(existing):
				combs[i][0] = 0
				probs[i] *= probability
	...
	def jump(self, state: State, transition: Transition, branch: Branch, expressions: List[int] = []) -> State:
		...
		for i in range(0, 1):
			...
			if transition.transitions[0] != -1:
				self.aut_Main.jump(state, transient, transition.transitions[0], branch.branches[0], i, target_state, target_transient)
			state = target_state
